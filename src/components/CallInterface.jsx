...
export default function CallInterface() {
    ...
    const isVideoCall = callerInfo?.callType === 'video' || (callStatus === 'ringing' && !isCameraOff);

    // 🔥 SYNC LOGIC: Remote status monitor
    useEffect(() => {
    ...
    // --- 1. INCOMING & OUTGOING CALL SCREENS ---
    if (callStatus === 'receiving' || callStatus === 'ringing') {
        const isReceiving = callStatus === 'receiving';
        const displayUser = isReceiving ? callerInfo : selectedFriend;
        const isAudioCall = isReceiving ? callerInfo?.callType === 'audio' : !isVideoCall;
        ...
        return (
            <div className="fixed inset-0 z-[999] bg-zinc-950 flex flex-col items-center justify-center animate-in zoom-in duration-300">
                <div className="relative z-10">
                    <img
                        src={displayUser?.photo || displayUser?.photoURL || `https://ui-avatars.com/api/?name=${displayUser?.name || 'U'}&background=random&color=fff&bold=true&length=1&uppercase=true`}
                        className="w-32 h-32 rounded-full border-4 border-blue-500 shadow-2xl mb-6 object-cover"
                        alt="Caller"
                        onError={(e) => {
                            e.target.src = `https://ui-avatars.com/api/?name=${displayUser?.name || 'U'}&background=random&color=fff&bold=true&length=1&uppercase=true`;
                        }}
                    />
                ...
                <h2 className="text-4xl font-black text-white italic mb-2 tracking-tighter z-10">{displayUser?.name || 'User'}</h2>
                <p className="text-blue-500 font-mono tracking-[0.3em] uppercase text-[10px] mb-16 animate-pulse z-10">
                    {isReceiving ? `Incoming ${callerInfo?.callType} Call...` : 'Calling...'}
                </p>
    ...
    return (
        <div className="fixed inset-0 bg-black z-[100] flex flex-col md:flex-row overflow-hidden animate-in fade-in duration-700">
            ...
            <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-[110] flex items-center gap-6 bg-black/40 backdrop-blur-xl px-6 py-3 rounded-full border border-white/10 shadow-2xl">
                <button onClick={toggleMic} className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl transition-all ${isMuted ? 'bg-zinc-800 opacity-80' : 'bg-white/10 hover:bg-white/20'}`}>
                    {isMuted ? '🔇' : '🎙️'}
                </button>
                {isVideoCall && (
                    <button onClick={toggleCamera} className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl transition-all ${isCameraOff ? 'bg-zinc-800 opacity-80' : 'bg-white/10 hover:bg-white/20'}`}>
                ...
                <button onClick={endCall} className="w-16 h-16 bg-red-600 rounded-full flex items-center justify-center text-3xl shadow-2xl active:scale-90 transition-transform hover:bg-red-700">
